...
class EnvironmentManager:
    ...
    def push_scope(self, type):
        self.scopes.append({"type": type, "variables": {}})
            ...
            for key, value in scope["variables"].items():
                ...
                if isinstance(value.value(), dict):
                    for key2, value2 in value.value().items():
                        print(
                            f"Scope {len(self.scopes) - i - 1} | {type} | {key} | {value.type()} | {key2}: {value2.type()}, {value2.value()}"
                        )
                else:
                    print(
                        f"Scope {len(self.scopes) - i - 1} | {type} | {key} | {value.type()} | {value.value()}"
                    )
        print()
